...
| Tesztek | Vitest | 45/45 teszt zöld |
...
A tesztek a `server/compliance.test.ts` és `server/auth.logout.test.ts` fájlokban vannak, Vitest keretrendszerrel. Jelenleg **45/45 teszt zöld**. A tesztek lefedik:
...
**G. (új) Embedding-backfill UI a `/regulations` oldalon (V12):** Jelenleg a `regulationSources.regenerateEmbeddings` és `knowledgeBase.regenerateEmbeddings` endpointok léteznek, de nincs UI gomb az adminoknak embeddings generáláshoz. A `/regulations` oldalt (ami jelenleg félrevezetően csak Tudástárt mutat — lásd alábbi drift) érdemes lenne ténylegesen regulation-source manager-ré bővíteni, "Embeddings generálása" gomb is mellé.

**H. (új, drift-fix) `/regulations` oldal félrevezető tartalma:** A `RegulationLibraryPage.tsx` a "Jogszabályok" menüpont mögött, de valójában `knowledgeBase.list` adatait mutatja, nem `regulationSources`-t. Vagy a route-ot kéne átkötni a tényleges Tudástár oldalra (`/knowledge-base`), vagy a komponenst átírni hogy `regulationSources` adatokat mutasson és CRUD műveleteket biztosítson. Az utóbbi az "G" feladat előfeltétele.
